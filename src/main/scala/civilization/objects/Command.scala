...
object Command extends Enumeration {
  ...
  BUYBUILDING, BUYWONDER, POTTERYACTION, PHILOSOPHYACTION,DEVOUTTOCULTURE,ADVANCECULTURE = Value
  ...
  def actionPhase(t: Value): TurnPhase.T = {
    t match {
      ...
      case BUYARMY | BUYSCOUT | SPENDTRADE | UNDOSPENDTRADE | SENDPRODUCTION | UNDOSENDPRODUCTION |
           BUYMOUNTED | BUYINFANTRY | BUYAIRCRAFT | BUYARTILLERY | HARVESTRESOURCE | BUYBUILDING | BUYWONDER | POTTERYACTION | PHILOSOPHYACTION | DEVOUTTOCULTURE | ADVANCECULTURE => TurnPhase.CityManagement
