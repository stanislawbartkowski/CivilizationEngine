...
object Command extends Enumeration {
  ...
  def actionPhase(t: Value): TurnPhase.T = {
    t match {
      ...
      case BUYARMY | BUYSCOUT | SPENDTRADE | UNDOSPENDTRADE | SENDPRODUCTION | UNDOSENDPRODUCTION |
           BUYMOUNTED | BUYINFANTRY | BUYAIRCRAFT | BUYARTILLERY | HARVESTRESOURCE | BUYBUILDING | BUYWONDER | POTTERYACTION | PHILOSOPHYACTION | DEVOUTTOCULTURE | ADVANCECULTURE | CURRENCYACTION | DISCARDCARD => TurnPhase.CityManagement
  ...
  def actionInCity(t: Value): Boolean = (t != ADVANCECULTURE) && (t != DISCARDCARD)
