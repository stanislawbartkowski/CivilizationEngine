...
class Test32 extends AnyFunSuite with ImplicitMiximToJson with ImplicitMiximFromJson {
  ...
  test("MetalCasting action") {
    ...
    val ince1 = gg.playerDeck(Civilization.China).resou.nof(Resource.Incense)
    println(ince1)
    println(l)
    assert(ince1 == 1)
